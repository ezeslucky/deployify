...
export const ai = pgTable("ai",{
    aiId: text("aiId").notNull().primaryKey().$default(() => nanoid()),
...
export const aiRelations = relations(ai, ({
    one
}) => ({
...
export const apiCreateAi = createSchema.pick({
    name: true,
    apiUrl:true,
    apiKey: true,
    model: true,
...
export const apiUpdateAi = createSchema.partial().extend({
    aiId: z.string().min(1),
