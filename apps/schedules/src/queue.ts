...
export const getJobRepeatable = async( data: QueueJob): Promise<RepeatableJob | null> =>{
    ...
    const RepeatableJob = await jobQueue.getRepeatableJobs()
    if(data.type === "backup"){
        const {backupId} = data
        const job = RepeatableJob.find((j) => j.name === backupId)
return job ? job: null
    }

    if(data.type === "server"){
        const {serverId} = data
        const job = RepeatableJob.find((j) => j.name === `${serverId}-cleanup`)
        return job ? job : null
    }
